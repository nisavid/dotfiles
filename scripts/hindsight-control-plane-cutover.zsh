...
home_root="${HOME:A}"
installation_config="${HINDSIGHT_INSTALLATION_CONFIG:A}"
[[ "$installation_config" == "$home_root"/* &&
  -f "$installation_config" &&
  ! -L "$installation_config" ]] || exit 1
...
[[ "$manifest_managed_python" == "$home_root"/* &&
