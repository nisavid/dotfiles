...
def _matches_checked_in_adapter_schema(document: Any) -> bool:
    return _validate_schema(
        ...
        root_schema_name=ADAPTER_SCHEMA_NAME,
        allowed_schema_names={ADAPTER_SCHEMA_NAME, "catalog-v1.schema.json"},
    )
...
def _validate_canonical_capability(
    ...
    component_support = capability.get("component_control_support")
    supported_identities = (
        component_support.get("supported_equipment_identities")
        if isinstance(component_support, dict)
        else None
    )
    if isinstance(supported_identities, list) and supported_identities != sorted(
        supported_identities
    ):
        diagnostics.append(
            Diagnostic(
                "CAPABILITY_ORDER_INVALID",
                "CapabilityDiscovery.result.records[].component_control_support.supported_equipment_identities",
                "Supported component equipment identities must use canonical Unicode order.",
            )
        )
    ...
    if isinstance(records, list) and records != sorted(
        ...
        diagnostics.append(
            Diagnostic(
                ...
                "Capability records must use canonical harness, provider, selector, and identity order.",
            )
        )
    matches = (
        [
...
def _capability_sort_key(record: Any) -> tuple[str, ...]:
    ...
    selector = {
        "standalone_skill": (
            provider.get("canonical_root"),
            "",
        ),
        "native_plugin": (
            provider.get("manager"),
            provider.get("scope"),
        ),
        "direct_mcp": (
            provider.get("transport"),
            provider.get("overlay_family"),
        ),
    }.get(kind, ("", ""))
    return tuple(
