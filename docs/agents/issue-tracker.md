...
- **Child ticket**: an issue linked to the map as a GitHub sub-issue. Create new children with `gh issue create --parent <map>`; attach existing issues with `gh issue edit <n> --parent <map>` or `gh issue edit <map> --add-sub-issue <n>`. Fall back to the API only when those flags are unavailable: `gh api --method POST repos/<owner>/<repo>/issues/<map>/sub_issues -F sub_issue_id=<child-db-id>`, where the id is the child's numeric database id. Where sub-issues aren't enabled at all, add the child to a task list in the map body and put `Part of #<map>` at the top of the child body. Labels: `wayfinder:<type>` (`research`/`prototype`/`grilling`/`task`). Once claimed, the ticket is assigned to the driving dev.
...
- **Frontier query**: read the map's children in map order with one GraphQL query, then pick the first open, unassigned, unblocked child:
  ...
  ```sh
  gh api graphql -f owner=<owner> -f repo=<repo> -F map=<map-number> -f query='
