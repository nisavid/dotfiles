...
- **Frontier query**: list the map's open children (`gh issue list --state open --limit 200`, scoped to the map's sub-issues / task list; paginate past 200 children rather than trusting a truncated page), then drop any with an open blocker or an assignee; first in map order wins. Check blockers through exactly one response shape: with `gh api`, `.issue_dependencies_summary.blocked_by > 0` (open blockers only); with GraphQL `blockedBy`, filter nodes to `state == "OPEN"` rather than using `totalCount`, which also counts closed blockers. Do not mix the two shapes in one query, and include the `Blocked by:` body-line fallback when dependencies are unavailable.
