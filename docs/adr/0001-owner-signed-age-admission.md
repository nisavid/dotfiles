...
Receipt creation requires clean exact checkouts, imports its creator and verifier
modules only after binding the creator to the trusted base, and materializes
the candidate commit's Git tree before identity-backed validation. The signed
tree digests therefore describe the same bytes that local validation inspected.
