...
4. **Native rolling channels.** Production gate `LIVE-02` records Claude marketplace update controls and reinstall behavior. The route remains `native_rolling` unless the gate proves an exact fetched artifact and digest; unsupported operations remain `operator_action` or `unavailable`, with manager-driven drift reported explicitly.

## Design status

The decision-complete route is captured in the
[implementation handoff](../agent-equipment/IMPLEMENTATION_HANDOFF.md), which
orders the production controller, adapters, migration, MCP coverage, and
acceptance work. The handoff and its supporting documents are design artifacts,
not runtime configuration or migration authorization.
