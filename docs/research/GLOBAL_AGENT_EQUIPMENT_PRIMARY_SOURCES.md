...
The accepted decisions are recorded in the [Wayfinder map](../../.scratch/global-agent-equipment/map.md) and its resolved grilling tickets. The authored catalog supports both source-wide and explicit component selection; a reviewed lock records exact inventory and each provider's restore class. Ordinary apply restores pinned targets where a reproducible artifact route exists, while native-rolling providers remain explicit and cannot claim deterministic version restore. A separate update operation produces reviewable lock changes for immutable targets. The global `skills` lock remains useful import evidence, but it is neither the desired-state authority nor a sufficient fresh-home restore mechanism. Harness adapters select providers and component controls while preserving explicit manual, unsupported, and omitted outcomes.
...
4. **Native rolling channels.** The [Matt, MCP, and update-channel prototype](../../.scratch/global-agent-equipment/issues/14-prototype-matt-and-mcp-resolution.md) must test whether representative providers can suppress background updates and restore exact artifacts. Where they cannot, require an explicit native-rolling, manual, or unsupported outcome plus manager-driven-drift reporting instead of claiming immutable convergence.
