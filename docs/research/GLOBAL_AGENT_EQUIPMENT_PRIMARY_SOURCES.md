...
1. Each selected provider route for an equipment identity and harness has exactly one artifact and provenance owner: `skills`, a dedicated helper, a checked-in custom skill, a referenced external, or a harness plugin. One logical package may therefore use distinct Claude, Codex, and Cursor provider routes with separate restore evidence.
2. The coverage matrix selects one preferred provider route for each managed-provider or manually-managed-provider equipment identity and harness unless an explicit exception allows overlap; intentional-omission and unsupported coverage outcomes select no provider. A separate operation matrix records `automated`, `operator_action`, or `unavailable` for each selected provider route and operation without changing the coverage outcome. Plugin installation is not assumed to activate an indivisible bundle: the resolver first applies every documented harness control that can selectively enable or disable a plugin component, then treats only the remaining inseparable activation groups as atomic. For example, Codex exposes per-plugin-MCP enablement and tool policy, while Claude plugin MCP servers start with the enabled plugin and plugin hooks cannot be individually disabled. [Codex configuration schema](https://developers.openai.com/codex/config-schema.json), [Claude MCP documentation](https://code.claude.com/docs/en/mcp), and [Claude hooks reference](https://code.claude.com/docs/en/hooks).
...
4. **Native rolling channels.** The [Matt, MCP, and update-channel prototype](../../.scratch/global-agent-equipment/issues/14-prototype-matt-and-mcp-resolution.md) must test whether representative providers can suppress background updates and restore exact artifacts. Where they cannot, require an explicit native-rolling restore class and `operator_action` or `unavailable` operation disposition plus manager-driven-drift reporting instead of claiming immutable convergence.
