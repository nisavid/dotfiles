...
3. **SCITT is a standards seam now and a production dependency later, if qualified.** RFC 9942 and RFC 9943 became Proposed Standards in June 2026, and RFC 9995 supplies a generic COSE digest-binding mechanism that this profile applies to the exact JCS bytes of one `signature-envelope/v1`. The profile separately fixes the OpenPGP byte mapping and requires independent RFC 9980/#209 authority and admission verification. Reserve a typed SCITT evidence profile and publish disposable hostile fixtures before the public wire contract freezes. Keep live registration, service trust, and production receipt verification optional until the API, service, witness, retained-checkpoint, privacy, and post-quantum paths are qualified. SCITT remains evidence, never release authority.
...
| Generic transparency receipts | [RFC 9942](https://www.rfc-editor.org/rfc/rfc9942.html), [RFC 9943](https://www.rfc-editor.org/rfc/rfc9943.html), and [RFC 9995](https://www.rfc-editor.org/rfc/rfc9995.html) | Define the optional SCITT evidence profile and disposable exact-byte fixtures now; qualify a live adapter and service later. Missing production inputs include final API/profile behavior, witnessed consistency, retained roots, privacy and exit policy, and a compatible PQ path. |
...
### SCITT and COSE receipts: standards seam now, production later
...
**Disposition.** **Use only as optional external evidence. Define the standards profile and disposable fixtures now; keep production integration later.** Use an [RFC 9995](https://www.rfc-editor.org/rfc/rfc9995.html) COSE Hash Envelope to bind the SHA-512 digest and media type of the exact JCS bytes of one <code>signature-envelope/v1</code>, which indexes detached OpenPGP material. Cross-check byte length from the authoritative #209 descriptor, then independently verify RFC 9980/#209 authority and admission. Preserve the SCITT Signed Statement, SCITT receipt, and underlying authority object as distinct objects and trust domains. Seed the hostile corpus from the IETF working-group vectors before wire freeze. Add no live service, SCRAPI client, trust store, or ordinary-release dependency until the selected API/profile, service trust roots, witness policy, retention, privacy, exit, and PQ migration path are qualified. The [focused SCITT adoption audit](SCITT_CRYPTO_RELEASE_OPS_ADOPTION.md) defines the crosswalk and evidence gates.
...
**Conflicts and boundaries.** Registration proves neither that a statement is true nor that its signer had release authority. Service ordering is not necessarily release issuance order. SCITT's native statements are COSE, the selected RFC 9980 hybrid does not yet have a standardized COSE/SCITT profile, and a receipt still needs independently trusted statement-issuer and service keys, monitors or witnesses, retained checkpoints, and a privacy decision. A receipt cannot upgrade <code>rejected</code>, <code>indeterminate</code>, or <code>attributed-historical</code> to <code>accepted-current</code>.

**Migration cost.** Low for the generic evidence slot and frozen fixtures, medium for a read-only adapter and retained validation bundle, high for making SCITT a required release service or authority plane.
...
- A reserved SCITT evidence profile using the registered media types and an RFC 9995 SHA-512 Hash Envelope over exact JCS <code>signature-envelope/v1</code> bytes, with the profile-defined OpenPGP mapping and independent RFC 9980/#209 verification, plus public disposable positive and hostile fixtures. This is a format and conformance surface, not a network or service dependency.
...
- Production SCITT receipt verification, SCRAPI service submission, live service trust, monitoring, and witnessing. The profile and disposable corpus belong in the first-release contract work.
...
| --- | --- |
| [#203, “Research historical verification, transparency, and trusted timestamps”](https://github.com/nisavid/dotfiles/issues/203#issuecomment-5463564990) | Uphold the optional-service and non-authority disposition. Refine its schedule from “prototype later” to “standard profile and disposable fixtures now, production adapter later.” SCITT's standards are published, but the accepted decision correctly distinguishes standards maturity from a suitable witnessed service. Refine the RFC 3161 adapter contract separately with RFC 5816 certificate identification, retained validation material, and an explicit RFC 4998 renewal or no-long-term-promise choice. |
...
- What exact SCITT application profile makes two independent verifiers agree on RFC 9995 Hash Envelope support, the profile-defined exact-preimage binding, and the RFC 9942 receipt corpus?
- What concrete final API/profile, witness quorum, monitor, retention, privacy, exit, and PQ conditions would justify moving from disposable fixtures to a production adapter and qualified service?
