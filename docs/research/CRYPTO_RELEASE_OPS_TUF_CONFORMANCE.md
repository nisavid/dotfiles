...
| --- | --- | --- | --- |
| Root | One signed root subject has role type, specification version, positive version, expiry, keys, roles, and the consistent-snapshot choice. Roles must name root, targets, snapshot, and timestamp key IDs and thresholds of at least one. | The certifying OpenPGP identity, grants, authority state, root transition, and certificate projection jointly describe authority. No object carries the complete TUF root fields or role assignments. | Mandatory root metadata is absent. |
...
| Consistent snapshots | Optional. Root selects the mode. When enabled, non-timestamp metadata uses version-prefixed names, targets use hash-prefixed names, current timestamp is unversioned, and all root versions remain available. | All objects use SHA-512 addresses; publication uses immutable objects, exact graph links, compare-and-swap, and atomic retained transactions. | Similar race resistance, not TUF consistent snapshots. Choosing false would not excuse missing roles. |
...
TUF persists trusted role metadata and retains every root version. Release Trust retains the anchor, profile locks, highest accepted global and family generations and heads, highest trusted time, freshness observations, complete usable state, and observed forks. It atomically commits a complete graph. Valid competing successors remain evidence and yield indeterminate; they never overwrite the accepted chain.
...
| Key compromise | Separate online and offline roles, delegation, and configurable thresholds can limit one key; insecure one-key layouts remain allowed. | A release signer can sign its manifests and positive lifecycle transitions within scope. Status authority is subtractive only. This is weaker than compartmentalized multi-role TUF, and similar to threshold-one reused-key TUF. Compromise remains authoritative until detected. |
...
| Key retention and history | Every root version remains available; trusted versions support rollback checks. | Broader archives allow bounded old-signer attribution. This increases archive and parser obligations. Old keys never regain current authority; unknown compromise timing is indeterminate. |
...
Each semantic-negative fixture must remain cryptographically valid under a disposable fixture authority long enough to reach its intended policy check. Every case binds the exact request, expected result or refresh disposition, deterministic diagnostics, and retained-state delta.
...
| Publication, bootstrap, and limits | Corrupt or lagging mirror, compare-and-swap race, crash before commit or readback, same controller as two channels, channel or time replay, backward clock, oversized object, excessive history, graph depth, or fork count. | Retrieval failure is unavailable or indeterminate; authenticated contradiction or limit violation is rejected. Commit only an old or new complete transaction, preserve conflicts and time floor, and keep evidence bounded. |
...
The wire and conformance freeze in [issue 213](https://github.com/nisavid/dotfiles/issues/213) must:
