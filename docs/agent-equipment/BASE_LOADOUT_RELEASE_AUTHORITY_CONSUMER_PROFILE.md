...
Before any staging effect, the protected component rechecks the `platform-payload` SHA-512 identity over the exact package or bundle. Separately, it rechecks the `installed-closure-manifest` SHA-512 identity before the Base Loadout hook parses that manifest, measures every declared installed member as the manifest specifies, and recomputes the existing launcher identity and SHA-256 installed-manifest digest from the verified closure. The hook supplies [issue #121](https://github.com/nisavid/dotfiles/issues/121) the candidate-independent mapping among the two admitted artifact identities, the verified installed members, and those independently derived Base Loadout identities; the mapping is trust input, not an additional candidate-authored release-tuple stream. Each identity remains bound to its own bytes and role, and none is converted into or substituted for another.
