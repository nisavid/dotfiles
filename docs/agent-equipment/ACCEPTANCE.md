...
`PreparationBundle`, evidence-only `PreparationReceipt`, pre-mutation
`ApplyAuthorization`, capture-observation and prepared-action authority sets,
closed `CheckpointSetManifest`, public `CompensationAuthorization`,
`ReleaseArchiveManifest`, and terminal `ReleaseReceipt`; none grants
...
preserved as reviewed state without claiming automated authority and is excluded
...
| `CAT-07` | Require every authored provider route, referenced or not, to be structurally valid and source/provider/restore coherent with its current Source Manifest; expansion alone determines active routes. Require every active or retiring route to have exactly one route control owner, exact provider-and-distribution-bound provenance owner, restore class, activation group, native-update state, and disposition for every required operation. Bind retirements to an exact current or historical Source Manifest; history contains every required non-current manifest and no orphan. | Field-deletion, wrong-binding, destructive-target, conflicting-field, missing-history, orphan-history, `test_unreferenced_template_still_binds_current_source_manifest`, and `test_unreferenced_template_is_fully_structurally_validated` mutation fixtures |
...
| `CAT-13` | Compute canonical catalog, lock, Source Manifest, and authoritative membership-evidence digests independent of formatting and object-key order; reject every stale or incomplete binding before opening the checkpoint store. | Digest vectors, stale-manifest and membership mutations, and checkpoint-open spy |
...
| ID | Required fixture and assertion | Evidence |
| --- | --- | --- |
| `RES-01` | Resolve identical inputs repeatedly to byte-identical diagnostics, coverage, provider selections, operation matrices, overlays, candidate and mutation plans, and resolution digests. | Repeated-run golden digests |
...
| `CMD-01` | `status` reads runtime state and writes neither authored nor runtime state. No legacy command alias is accepted. | Filesystem and fake-manager before/after digest equality plus CLI rejection cases |
| `CMD-02` | `unmanaged` reads runtime state and the authored catalog without changing either, then emits canonical secret-free observation records only for equipment positively observed on the machine and absent from the catalog. It accepts all-scope or exact targets, requires a complete all-scope response, excludes cataloged operator-owned routes, keeps every runtime observation factual rather than a proposal, and replaces all raw adapter evidence objects with canonical digests before emission. It enforces the exact v1 Step 3 discovery limits before adapter invocation or output emission, including the controller-wide target-or-record and aggregate-byte ceilings across every harness report in the pass. | Observation-record golden, complete-scope and exact-target negatives, operator-owned exclusion, raw-evidence projection and re-admission negatives, recursive secret-canary scan, exact-bound regressions, and runtime/catalog digest equality |
| `CMD-03` | `add` requires no prior `unmanaged` invocation. In the same invocation it observes every exact target twice, revalidates the state and every binding, and emits one atomic complete catalog-and-lock proposal. Multi-target work is all-or-nothing. Each selected target must bind to exactly one unambiguous reviewed distribution whose authoritative Source Manifest lists that target's equipment identity and whose complete compatible templates match its evidence. Compatible targets in one atomic proposal may bind to different distributions; missing, ambiguous, or conflicting policy returns `ADD_AUTHORING_POLICY_REQUIRED`. It enforces the exact v1 discovery and proposal limits, including the controller-wide target-or-record and aggregate-byte ceilings across every harness report in each observation pass, and does not change runtime state. | Double-observation trace, binding-change matrix, multi-distribution composition and conflict cases, multi-target atomicity, policy-boundary negatives, exact-bound and full-pair validation, and runtime/authored-state digest equality |
| `CMD-04` | `update` follows omitted-default or explicit Git branch policy and omitted-`latest` or explicit native channel policy. Its untrusted port returns only closed request-bound resolution facts; the controller combines them with reviewed base policy to construct one complete authoritative Source Manifest per selection. It rejects resolver-supplied policy, prose, selected membership, membership evidence, or presealed manifests; requires an `npx` source package without a tag/version suffix, derives the one exact resolved invocation selector, and requires one exact `secret-exec -- npx` boundary when the invocation is secret-wrapped; expands source-wide membership, regenerates coverage, preserves retirement-bound history without orphans, validates the complete pair, and emits one atomic proposal without installation or runtime mutation. It enforces the exact v1 source-resolution, proposal, and expansion limits. | Fact-only response and hostile-extra negatives, default and explicit source-policy fixtures, npx package/channel separation plus one-selector and wrapped-route one-boundary negatives, reviewed-policy preservation, Source Manifest and membership digests, exact-bound and full catalog-and-lock validation, history preservation, and runtime/authored-state digest equality |
| `CMD-05` | `apply` treats an empty admitted action set as a terminal verified no-op. For a nonempty set, it rejects a missing, stale, replayed, expired, or incomplete plan, authorization, or authenticated preparation bundle during preclaim before any adapter call or checkpoint transition. #116 performs the final live comparison and durable nonce-claim composition before deterministic reconciliation. | Partial: production candidate-independent preparation, authenticated apply-preclaim rejection, and separate durable ledger storage plus a model-only ordered plan trace; final live comparison, nonce-claim composition, and production executor/checkpoint evidence remain required |
...
| `CON-04` | Classify immutable runtime state only from freshly verified installed content and integrity-bound provenance. Treat an exact observed revision and content digest as converged, confirmed absence as installable, a known mismatch as restorable, and unknown evidence as ineligible for mutation. Restore only after target digest verification; reject corrupt content before replacing runtime state. | Exact/absent/revision-drift/content-drift/unknown matrix plus valid/corrupt artifact fixtures |
| `CON-05` | Switch a preferred provider route and retire only catalog-owned losing projections whose provider, physical surface, and restore evidence match the selected losing distribution, after the winner verifies. | Ordered switch trace, destructive-target negatives, and retained-unmanaged assertion |
...
| `CON-07` | Preserve unknown and unmanaged equipment. Exclude cataloged operator-owned routes from unmanaged results. Retirement of unmanaged equipment is a report only and performs no delete. | Observation filter and adapter delete spy remains zero |
| `CON-08` | Retire catalog-owned state only through apply, preserving unrelated keys and runtime objects. | Narrow-diff assertion |
| `CON-09` | Detect manager-driven native-rolling version drift. Ordinary apply does not advance the baseline; reviewed update proposes one semantically validated catalog-and-lock pair. | Drift diagnostic and paired proposal |
...
| `CON-11` | Preserve runtime drift encountered between planning and apply; compare-before-mutate stops rather than overwriting it. | Concurrent-change injection for every adapter surface |
...
| `ADP-05` | Keep desired target fragments distinct from complete normalized pre/post state. A manifest-bound read-only `prepare` response is closed `PreparedStateFacts`: it echoes exact request and adapter bindings and carries complete normalized pre/post state with self-digests, but no scope, controlled components, authority tuple, or mutation capability. Require exact capture, immediate guard, post-verification, and compensation restore digests. | Partial-as-full, noncanonical or misbound facts, post-state, and compensation negatives |
| `ADP-06` | Permit a `PreparationGate` to call only the exact read-only `prepare` seam in the closed adapter-manifest set. #116 may construct one closed mutating `ApplySequence` only after `ApplyPreclaimGate` has admitted the already issued authorization and authenticated preparation evidence, and only after the final live comparison and durable nonce claim. The sequence binds `command: apply`, the admitted authorization and issuer-derived preparation and authority-set tuples, an independently trusted current candidate identity and installed-manifest digest, the exact phase, durable checkpoint reference, action, receipt, and verification records. | Manifest substitution, foreign-candidate, caller-tuple, and coordinated authority counterexample matrix |
...
| `ADP-09` | Require one closed `immutable_content` tag in every normalized state. Immutable present state admits a freshly verified observed revision and content digest or truthful unknown; absence requires `route_absent`; partial or unknown presence requires unknown. Immutable version, native-update, suppression, and drift fields are `not_applicable`; native-rolling immutable content is `not_applicable`. Never accept copied request or lock values as observation evidence. | Closed-tag, route-coherence, capability-field, exact-post-state, and compensation-preservation matrix |
...
| `CAP-02` | Recompute the closed pre-capture plan-action-set, action, identity, desired-state-fragment, and set digests; bind the distinct candidate identity and installed-implementation manifest digest; require exact independently supplied current-candidate trust and authoritative action membership. The candidate-independent gate derives complete canonically ordered capture and prepared sets from exact capture, capability, and adapter-manifest bindings; each prepared pre-state equals its capture member. Complete normalized post-state and its digest belong only to the separately sealed `PreparedActionAuthoritySet`. | Canonical projection, foreign-candidate, coordinated substitution, forged-membership, manifest-binding, and capture/prepared mismatch matrix |
...
| `CAP-06` | Permit native inverse removal only for captured absence, an authoritative forward install that owns the same exact target, and the matching prepared member's complete `expected_post_state_digest` as its forward-post guard. | Presence/recovery/action-target coherence and prepared-guard mismatch matrix |
...
| ID | Required fixture and assertion | Evidence |
| --- | --- | --- |
| `CHK-01` | Before authorization, require the candidate-independent gate to commit one complete content-addressed preparation bundle and evidence-only receipt atomically in its create-only producer-owned store. The issuer resolves and rehashes those exact bytes through the authenticated producer path, derives the bundle and both authority-set tuples, and issues the complete authorization. `ApplyPreclaimGate` then validates that already issued authorization against independent trust, re-resolves its exact bundle digest, and revalidates every artifact. #116 performs the final live comparison and durably claims the fresh execution nonce in the trusted domain before the first action checkpoint. Static preparation rejection makes zero adapter calls; invalid read-only facts make zero mutating calls and checkpoint transitions; apply-admission rejection makes zero adapter calls of any kind and zero checkpoint transitions. | Static-preflight, invalid-facts, receipt-resolution, authorization, last-action-invalid, and nonce-order fixtures |
...
| `CHK-03` | Persist `prepared` with invocation intent `not_started`, fail its atomic transition to `started`, prove no adapter call occurred, then obtain a fresh read-only status observation and retry once without destructive replay. | Recovery classification, intent-write fault, and call counts |
| `CHK-04` | Persist `prepared` with invocation intent `started`, complete the mutation, and fail before completion persistence. Retry observes the expected post-state and records completion without replay. A `not_started` record at the same target is concurrent drift, never this run's effect. | Mutation receipt, state digest, and call counts |
...
| `CHK-08` | Fail compensation and preserve a durable recoverable record. A fresh read-only status observation before retry classifies state and never issues duplicate or destructive replay. | Fault and retry trace |
...
| `CHK-10` | Before apply issuance, the separately packaged `PreparationGate` validates static inputs before adapter calls, invokes only exact manifest-bound read-only `prepare`, derives and seals one complete nonempty `CaptureObservationAuthoritySet` over all-and-only plan actions, and derives one matching complete nonempty `PreparedActionAuthoritySet` from closed `PreparedStateFacts`. It atomically commits their exact bytes, plan/capture/capability and exact adapter/gate manifests in one content-addressed bundle and evidence-only receipt. The issuer resolves and rehashes that receipt and bundle through the authenticated producer-owned store, derives both expected tuples, and binds the preparation-bundle digest, both tuples, and operator-review-package digest into `ApplyAuthorization`; a coordinated reseal requires new apply authority. Every public validation seam takes both closed artifacts and gets both expected tuples from that authorization; no raw observation list or caller tuple is accepted. Bind each checkpoint identity to that prepared authority plus apply identity/digest, execution domain/nonce/run, candidate and implementation, catalog/lock/plan/capability/capture, route capability/manager evidence, action/ordinal/step, operation, surface, compensation, and exact normalized pre/post state. Require checkpoints to form the canonical prepared plan prefix and one reachable cross-record forward or reverse-topological lifecycle frontier in addition to the local phase-history x phase x invocation-state matrix. Distinguish automatic apply compensation from public compensation; the latter requires a closed non-null original-authority transition claim and durable ledger evidence for restart. An empty admitted action set is terminal verified no-op and creates none of these artifacts. | Capture/prepared-set closure, static-before-call, manifest binding, bundle/receipt atomicity, reuse/retry/durability, producer-store resolution, coordinated reseal, cross-module identity, malformed normalized-state, component-set, phase-matrix, complete-plan, prefix/frontier, claim-type, and compensation-restart negatives |
...
| `LIVE-06` | Native manager drift. | Change or observe a rolling provider version and prove `status` reports drift while `update` alone proposes the reviewed baseline advancement. |
...
| Independent authority and complete binding | `test_execution_binding_is_post_authorization_and_externally_trusted`; `test_trusted_attestation_freezes_every_candidate_bundle_value`; `test_candidate_cannot_reseal_bundle_and_attestation_under_old_trust`; `test_fully_resealed_artifact_tuple_requires_a_new_attestation_trust_root`; `test_forged_attestor_fails_under_the_original_trusted_digest`; `test_attestation_must_follow_results_and_bind_the_live_signer`; `test_timestamp_ordering_handles_nanosecond_fractional_precision`; `test_bindings_reject_foreign_artifacts_and_route_capability_substitution`; `test_route_capability_bindings_close_each_provider_family_tuple`; `test_trusted_manifest_digest_rejects_a_coordinated_foreign_tuple` |
...
The preparation bundle and receipt, capture-observation-authority set,
prepared-action-authority set, apply authorization, checkpoint-store snapshot,
checkpoint-set manifest, compensation authorization and transition claim,
run-terminal record, release archive manifest, and release receipt are
preparation or execution records, not children inside the candidate's 75-ID
evidence bundle. The methods in `AgentEquipmentDeploymentContractTests` pin
their closed source shape and the future deployment separation:

| Gate family | Exact test methods |
| --- | --- |
| Preparation authority | `test_preparation_bundle_binds_all_exact_pre_authorization_streams`; `test_preparation_receipt_is_closed_commit_evidence_not_apply_authority` |
...
| Capture-observation authority | `test_capture_observation_authority_is_closed_and_apply_bound`; `test_capture_observation_reseal_cannot_escape_apply_authority`; `test_capture_observation_authority_rejects_resealed_preparation_manifest_substitution`; `test_capture_observation_authority_rejects_partial_raw_and_secret_inputs` |
| Prepared-state authority | `test_prepared_action_authority_is_closed_over_preparation_evidence`; `test_prepared_action_authority_is_complete_and_semantically_bound`; `test_prepared_action_authority_rejects_coordinated_reference_substitution`; `test_prepared_action_authority_rejects_component_and_data_ambiguity`; `test_prepared_action_authority_requires_exact_controlled_components` |
...
| V1 compatibility boundary | `test_v1_foundation_exception_is_scoped_and_freezes_after_merge` |

These are pure design-contract vectors. They prove closed preparation-bundle
and receipt shape, canonical identities and digests, complete apply-bound
capture/prepared authority equality, bounded strict raw parsing, apply-time
trusted-clock ordering, semantic revalidation of all eleven release streams
against the trusted release tuple, byte-owned archive digest recomputation, and
receipt-to-archive equality. Production must
...
| `CAT-10` | `AgentEquipmentDesignTest.test_automated_mutation_requires_pre_state_compensation`; `AgentEquipmentDesignTest.test_an_invalid_final_entry_yields_no_plan`; `ResolverMatrixTest.test_unsupported_catalog_automation_is_fatal_before_planning`; `ResolverMatrixTest.test_unsupported_automated_action_closes_cat_10_end_to_end` prove catalog compensation, exact capability matching, and full-resolution fail-closed behavior with no candidate or mutation plan. |
| `CAT-11`, `CAT-12` | `AgentEquipmentDesignTest.test_immutable_restore_requires_revision_reference_digest_and_update_control`; `AgentEquipmentDesignTest.test_immutable_restore_binds_a_commit_selector_to_its_revision`; `AgentEquipmentDesignTest.test_native_rolling_restore_requires_reviewed_update_state`; `AgentEquipmentDesignTest.test_native_update_control_matches_suppression_disposition` |
| `CAT-13`, `CAT-14` | `AgentEquipmentDesignTest.test_canonical_digest_is_utf8_compact_and_key_sorted`; `AgentEquipmentDesignTest.test_stale_catalog_lock_digest_is_rejected`; `CatalogLockValidatorTests.test_source_manifest_semantics_reject_invalid_resolution_evidence`; `AgentEquipmentDesignTest.test_checked_in_source_manifests_are_complete_and_digest_bound`; `AgentEquipmentDesignTest.test_provider_configuration_is_typed_and_secret_safe`; `AgentEquipmentDesignTest.test_literal_secret_material_fails_closed_without_echoing_it`; `AgentEquipmentDesignTest.test_secret_references_accept_environment_variables_and_opaque_profiles` |
| `ADP-01` | `AdapterContractSchemaTests.test_schema_is_valid_draft_2020_12`; `AdapterContractSchemaTests.test_valid_records_satisfy_the_closed_contract`; `AdapterContractSchemaTests.test_invalid_records_fail_closed`; `AdapterContractSchemaTests.test_capability_binding_set_preserves_semantic_and_artifact_digests`; `AdapterContractSchemaTests.test_capability_declares_the_manifest_bound_prepare_record_versions`; `AdapterContractSchemaTests.test_individual_record_does_not_grant_mutation_authority`; `PreparationAuthorityTests.test_gate_uses_one_immutable_schema_view_for_validation_and_manifest`; `ImmutableContentSchemaTests.test_authoritative_and_installed_schemas_are_valid_and_digest_pinned` |
...
| `ADP-03` | `AdapterContractSchemaTests.test_valid_sequence_has_canonical_embedded_payload_digests_and_exact_bindings`; `AdapterContractSchemaTests.test_prepare_request_binds_one_exact_action_capture_and_adapter`; `AdapterContractSchemaTests.test_apply_sequence_recomputes_action_identity_coordinates`; `AdapterContractSchemaTests.test_sequence_rejects_each_request_observation_echo_mismatch`; `AdapterContractSchemaTests.test_sequence_rejects_each_request_action_and_action_receipt_echo_mismatch` |
...
| `ADP-05` | `AdapterContractSchemaTests.test_prepared_state_facts_echo_bindings_without_granting_scope`; `PreparationAuthorityTests.test_prepared_facts_cannot_add_foreign_controlled_components`; `PreparationAuthorityTests.test_configure_preparation_accepts_only_observed_desired_configuration`; `AdapterContractSchemaTests.test_apply_sequence_rejects_desired_fragment_digest_as_full_state_digest`; `AdapterContractSchemaTests.test_apply_sequence_rejects_noncanonical_pre_capture_state_digest`; `AdapterContractSchemaTests.test_sequence_rejects_ok_state_and_compensation_restore_mismatches`; `AdapterContractSchemaTests.test_sequence_accepts_compensation_restored_to_captured_pre_state` |
| `ADP-06`, `ADP-07` | `AdapterContractSchemaTests.test_preparation_gate_manifest_is_separate_and_content_bound`; `AdapterContractSchemaTests.test_preparation_adapter_manifest_set_is_one_closed_manifest_bound_seam`; `PreparationAuthorityTests.test_each_action_gets_one_manifest_bound_prepare_call`; `PreparationAuthorityTests.test_gate_rejects_an_adapter_handle_that_exposes_mutation`; `PreparationAuthorityTests.test_gate_calls_the_prepare_capability_bound_at_construction`; `PreparationAuthorityTests.test_gate_uses_store_capabilities_bound_at_construction`; `PreparationAuthorityTests.test_receipt_is_authenticated_to_its_producer_owned_store`; `AdapterContractSchemaTests.test_valid_apply_sequence_satisfies_the_closed_contract`; `AdapterContractSchemaTests.test_apply_sequence_rejects_coordinated_authority_counterexamples`; `AdapterContractSchemaTests.test_apply_sequence_rejects_verification_of_a_different_route`; `AdapterContractSchemaTests.test_sequence_rejects_native_rolling_plugin_remove`; `AdapterContractSchemaTests.test_cross_field_safety_invariants_fail_closed`; `AdapterContractSchemaTests.test_public_sequence_rejects_literal_credentials_without_echoing_them` |
...
| Partial `ADP-09` | `ImmutableContentSchemaTests.test_normalized_state_requires_closed_immutable_content_evidence`; `ImmutableContentSchemaTests.test_observed_version_accepts_closed_not_applicable_tag`; `AdapterContractSchemaTests.test_normalized_state_requires_one_closed_immutable_content_tag`; `AdapterContractSchemaTests.test_immutable_route_admits_exact_and_truthfully_unknown_prestate`; `AdapterContractSchemaTests.test_restore_classes_reject_crossed_observation_tags`; `AdapterContractSchemaTests.test_immutable_observation_presence_tags_must_cohere`; `AdapterContractSchemaTests.test_immutable_route_capability_must_advertise_content_observation`; `AdapterContractSchemaTests.test_verified_immutable_post_state_binds_revision_and_content_digest`; `AdapterContractSchemaTests.test_compensation_preserves_captured_immutable_content` prove closed shape and coherence. A Step 5 production adapter fixture must derive revision and content from installed provenance and bytes, prove no request or lock echo, and return unknown when that proof is unavailable. |
...
| `CAP-02` | `CapturedStateValidationTest.test_plan_action_digest_covers_the_closed_canonical_payload`; `CapturedStateValidationTest.test_full_plan_projection_binds_all_automated_action_authority`; `CapturedStateValidationTest.test_forward_install_requires_separate_authoritative_plan_membership`; `PreparationAuthorityTests.test_static_preflight_rejects_invalid_plan_semantics`; `PreparationAuthorityTests.test_static_preflight_rejects_invalid_capability_inventory`; `PreparationAuthorityTests.test_one_action_prepares_and_commits_one_complete_bundle`; `PreparationAuthorityTests.test_noncanonical_prepared_facts_make_no_store_commit`; `AgentEquipmentDeploymentContractTests.test_prepared_action_authority_is_complete_and_semantically_bound` |
| `CAP-03` | `CapturedStateValidationTest.test_plan_action_projection_represents_secret_safe_mcp_providers` |
| `CAP-04`, `CAP-05` | `CapturedStateValidationTest.test_action_write_scope_and_canonical_dependencies_are_bijective`; `CapturedStateValidationTest.test_schema_rejects_skill_root_and_ownership_conflation`; `CapturedStateValidationTest.test_full_plan_projection_binds_all_automated_action_authority`; `PreparationAuthorityTests.test_static_preflight_rejects_an_unbound_mutable_captured_surface`; `PreparationAuthorityTests.test_static_preflight_rejects_invalid_closed_capture_bindings` |
| `CAP-06` | `CapturedStateValidationTest.test_native_installation_capture_accepts_only_coherent_route_evidence`; `CapturedStateValidationTest.test_forward_install_requires_separate_authoritative_plan_membership`; `PreparationAuthorityTests.test_native_remove_guard_must_equal_prepared_expected_post_state` |
| `CAP-07` | `CapturedStateValidationTest.test_manifest_semantics_fail_closed`; `CapturedStateValidationTest.test_malformed_manifest_returns_a_diagnostic_instead_of_crashing`; `CapturedStateValidationTest.test_diagnostics_never_echo_captured_identifiers`; `CapturedStateValidationTest.test_cli_exits_nonzero_for_a_semantic_error`; `CapturedStateValidationTest.test_public_capture_rejects_literal_credentials_without_echoing_them`; `CapturedStateValidationTest.test_public_plan_action_set_rejects_literal_credentials_without_echoing_them`; `PreparationAuthorityTests.test_producer_secret_policy_matches_the_controller_corpus`; `PreparationAuthorityTests.test_external_failures_return_one_secret_free_rejection` |
| `RES-01` | `AgentEquipmentDesignTest.test_resolution_is_deterministic_under_input_ordering`; `ResolverMatrixTest.test_complete_inventory_resolves_deterministically_without_mutation_authority`; `ResolverGraphTest.test_input_permutations_produce_identical_plan_bytes_and_identities` |
| `RES-02` | `AgentEquipmentDesignTest.test_component_controls_have_one_exact_non_conflicting_shape`; `AgentEquipmentDesignTest.test_shared_activation_group_produces_one_action_per_route_operation`; `AgentEquipmentAcceptanceTest.test_route_switch_controls_components_and_retires_only_owned_duplicates`; `ResolverMatrixTest.test_active_groups_preserve_complete_coverage_and_apply_controls_first`; `ResolverMatrixTest.test_selected_component_controls_require_exact_capability_support` |
| `RES-03`, `RES-04` | `AgentEquipmentDesignTest.test_proposed_initial_catalog_and_lock_are_complete_and_valid`; `AgentEquipmentDesignTest.test_initial_inventory_counts_and_classifications_are_complete`; `ResolverMatrixTest.test_every_active_and_retiring_route_selects_one_exact_capability`; `ResolverMatrixTest.test_only_reviewed_retirement_records_become_losing_route_groups`; `ResolverMatrixTest.test_provider_switch_graph_verifies_winner_before_every_losing_retirement` |
| `RES-05` | `AgentEquipmentDesignTest.test_provider_configuration_is_typed_and_secret_safe`; `AgentEquipmentAcceptanceTest.test_durable_artifacts_contain_secret_references_but_no_secret_values`; `ResolverMatrixTest.test_complete_inventory_resolves_deterministically_without_mutation_authority`; `StatusCliTests.test_status_seam_redacts_a_literal_credential_before_rendering` |
| Partial `CMD-01`, `CMD-02`, and `CMD-03` | `AuthoredCommandCliTests.test_only_the_five_public_v1_command_grammars_are_recognized` proves current grammar and legacy-alias rejection. `StatusCliTests.test_exact_status_ignores_xdg_and_resolves_reviewed_home_inputs_read_only` and the read-only collector tests prove status. `EquipmentDiscoveryRequestTests` and `EquipmentDiscoveryCollectionTests` prove exact/all-scope discovery, bounds, canonicalization, atomicity, and secret rejection. `UnmanagedAuthoringTests` prove factual catalog-absent filtering. `AddAuthoringTests` prove no-prior-run semantics, two-pass revalidation, the narrow policy boundary, multi-target atomicity, complete-pair validation, and historical retirement preservation. `AgentEquipmentDeploymentContractTests.test_step3_resource_bounds_are_frozen_in_contract` binds the exact limits to the v1 prose. Installed discovery adapters remain Step 5 work, so live registry evidence is still required. |
| Partial `CMD-04` | `SourceResolutionRequestTest` and `SourceResolutionFactAdmissionTest` prove closed request binding, fact-only responses, manager-typed exact versions, authoritative membership, reviewed restore-policy preservation, controller-derived Source Manifests, and hostile-extra rejection. `UpdateProposalTest` proves all-or-one deterministic resolution, complete source-wide expansion and coverage regeneration, retirement-bound history without orphans, full-pair validation, bounds, and atomic failure. `AgentEquipmentDeploymentContractTests.test_step3_resource_bounds_are_frozen_in_contract` binds the exact limits to the v1 prose. `CatalogLockValidatorTests.test_source_manifest_semantics_reject_invalid_resolution_evidence`, `CatalogLockValidatorTests.test_retirement_binds_exact_history_and_rejects_missing_or_orphan_history`, `AgentEquipmentJsonSchemaTests.test_source_manifest_completeness_history_and_digest_fields_are_required`, and `AgentEquipmentDesignTest.test_checked_in_source_manifests_are_complete_and_digest_bound` bind the serialized contract. An installed sandboxed resolver that proves the live default branch or `latest` result, consumes only public source metadata, and binds source-specific provenance remains required. |
| Partial `CMD-05` | `PreparationAuthorityTests.test_one_action_prepares_and_commits_one_complete_bundle`; `PreparationAuthorityTests.test_gate_uses_store_capabilities_bound_at_construction`; `PreparationAuthorityTests.test_receipt_is_authenticated_to_its_producer_owned_store`; `ApplyPreclaimAdmissionTest.test_admission_reloads_and_rehashes_authenticated_bundle_before_cas`; `ApplyPreclaimAdmissionTest.test_admission_uses_the_resolver_bound_at_construction`; `ApplyPreclaimAdmissionTest.test_untrusted_authorization_bindings_never_reach_the_resolver`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_add_an_unbound_mutable_surface`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_open_the_captured_route_graph`; `FileAuthorizationLedgerTest.test_module_has_no_direct_apply_start_api`; `FileAuthorizationLedgerTest.test_file_ledger_durably_claims_once_and_preserves_replay_evidence`; `FileAuthorizationLedgerTest.test_concurrent_claims_have_exactly_one_durable_winner`; `PlanActionSetAdmissionTest.test_complete_projection_is_admitted_as_one_immutable_typed_artifact`; `PlanActionSetAdmissionTest.test_missing_or_extra_actions_cannot_change_complete_membership`; `PlanActionSetAdmissionTest.test_trusted_set_digest_is_independent_of_the_supplied_projection`; `PlanActionSetAdmissionTest.test_each_exact_surface_identity_rule_admits_only_its_derived_set`; `PlanActionSetAdmissionTest.test_direct_constructor_cannot_bypass_complete_trust_membership`; `PlanActionSetAdmissionTest.test_legacy_projector_fails_closed_until_locator_authority_exists`; the apply-authorization deployment-contract tests; and the invalid-final-action and fresh-home acceptance-model fixtures. Production proves candidate-independent preparation, authenticated preclaim admission of the complete preparation bundle, a separate durable nonce-ledger storage primitive with no direct apply-start API, and strict admission of a complete supplied plan-action-set against independent plan and set-digest trust for supported physical target kinds. Legacy-projector targets remain unavailable until their exact locator and route-slot authority are durably specified. Final live comparison, authorized composition with the nonce claim, production projection of the artifact, executable-action authority, action checkpoints, and ordered reconciliation remain unproved. |
...
| Partial `CON-04` | `AgentEquipmentAcceptanceTest.test_immutable_content_is_verified_before_explicit_update_mutates` proves target artifact verification before replacement. `ResolverMatrixTest.test_immutable_active_content_is_exactly_reconciled_and_verified`; `ResolverMatrixTest.test_immutable_and_native_rolling_evidence_cannot_cross_classes`; `ResolverMatrixTest.test_immutable_retirement_deletes_only_exact_reviewed_content`; `ResolverMatrixTest.test_immutable_evidence_changes_plan_digest_but_not_permutation_result`; `ResolverMatrixTest.test_immutable_selection_requires_content_inspection_capability` prove exact, absent, mismatched, and unknown classification; exact-only retirement; evidence-bound deterministic planning; and selection of content-inspection authority. A Step 5 production adapter fixture must derive revision and content from installed provenance and bytes, prove no request or lock echo, and return unknown when that proof is unavailable. |
...
| Partial `CHK-01`; `CHK-02`, `CHK-03` | `PreparationAuthorityTests.test_static_preflight_rejects_before_any_prepare_or_store_call`; `PreparationAuthorityTests.test_invalid_facts_stop_after_one_read_only_call_without_commit`; `PreparationAuthorityTests.test_receipt_is_authenticated_to_its_producer_owned_store`; `ApplyPreclaimAdmissionTest.test_admission_reloads_and_rehashes_authenticated_bundle_before_cas`; `ApplyPreclaimAdmissionTest.test_untrusted_authorization_bindings_never_reach_the_resolver`; `FileAuthorizationLedgerTest.test_module_has_no_direct_apply_start_api`; `FileAuthorizationLedgerTest.test_file_ledger_keeps_one_cas_target_after_path_replacement`; `FileAuthorizationLedgerTest.test_concurrent_claims_have_exactly_one_durable_winner`; `PlanActionSetAdmissionTest.test_complete_projection_is_admitted_as_one_immutable_typed_artifact`; `PlanActionSetAdmissionTest.test_coordinated_reseal_cannot_substitute_validated_action_authority`; `PlanActionSetAdmissionTest.test_physical_targets_require_the_canonical_identity_order`; `PlanActionSetAdmissionTest.test_direct_mcp_selection_contract_matrix_is_complete`; `PlanActionSetAdmissionTest.test_legacy_projector_fails_closed_until_locator_authority_exists`; the apply-authorization deployment-contract tests; `AgentEquipmentDeploymentContractTests.test_runtime_gate_and_external_authority_precede_every_mutation`; and the invalid-plan and prepared-write acceptance-model fixtures. Production closes the candidate-independent preparation, authenticated evidence-only apply-preclaim, separate durable nonce-ledger storage primitive, and strict supplied plan-action-set admission portions of `CHK-01` for supported target kinds. #116 still owns the final live comparison and authorized composition of preclaim, one-time nonce claim, and zero-checkpoint execution admission. Legacy-projector locator and route-slot authority, resolver-to-action-set projection, executable-action authority, durable action-checkpoint writes, retry classification, and adapter-call evidence remain required. |
...
| `CHK-09`; Partial `CHK-10` | `PreparationAuthorityTests.test_empty_action_set_is_a_terminal_verified_noop`; `PreparationAuthorityTests.test_gate_rejects_an_adapter_handle_that_exposes_mutation`; `PreparationAuthorityTests.test_byte_identical_bundle_is_reused_only_after_binding_revalidation`; `PreparationAuthorityTests.test_reuse_rejects_every_changed_trust_and_deployment_binding`; `PreparationAuthorityTests.test_literal_secrets_in_prepared_facts_make_no_store_commit`; `PreparationAuthorityTests.test_gate_rejects_schema_bytes_not_bound_by_its_manifest`; `PreparationAuthorityTests.test_gate_uses_store_capabilities_bound_at_construction`; `PreparationAuthorityTests.test_store_requires_an_already_provisioned_protected_root`; `PreparationAuthorityTests.test_store_rejects_a_symlink_root`; `PreparationAuthorityTests.test_store_concurrently_reuses_only_the_same_complete_envelope`; `PreparationAuthorityTests.test_durability_uncertainty_returns_no_receipt_and_retry_revalidates`; `PreparationAuthorityTests.test_partial_or_corrupted_store_entry_cannot_resolve`; `ApplyPreclaimAdmissionTest.test_admission_uses_the_resolver_bound_at_construction`; `ApplyPreclaimAdmissionTest.test_admission_uses_only_the_import_time_captured_schema_set`; `ApplyPreclaimAdmissionTest.test_each_exact_artifact_stream_is_independently_schema_validated`; `ApplyPreclaimAdmissionTest.test_every_artifact_stream_is_rehashed_from_its_exact_bytes`; `ApplyPreclaimAdmissionTest.test_coordinated_outer_reseal_cannot_hide_an_invalid_artifact_self_digest`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_hide_schema_invalid_captured_state`; `ApplyPreclaimAdmissionTest.test_adapter_manifest_capability_substitution_is_rejected`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_replace_prepare_with_a_mutating_seam`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_add_an_unused_adapter_manifest`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_reorder_gate_schema_inventory`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_duplicate_gate_file_inventory`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_change_the_v1_store_generation`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_substitute_a_target_surface`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_substitute_a_dependency_surface`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_reorder_write_targets`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_substitute_a_target_locator`; `ApplyPreclaimAdmissionTest.test_coordinated_reseal_cannot_add_unrepresented_equipment`; `ApplyPreclaimAdmissionTest.test_cross_substituted_observation_is_rejected_after_full_reseal`; `ApplyPreclaimAdmissionTest.test_literal_secret_in_artifact_is_rejected_after_outer_reseal`; `ApplyPreclaimAdmissionTest.test_closed_preparation_schema_rejects_boolean_store_generation`; `ApplyPreclaimAdmissionTest.test_noncanonical_authorization_is_rejected_without_resolution`; `ApplyPreclaimAdmissionTest.test_coordinated_noncanonical_receipt_and_bundle_are_rejected`; `ApplyPreclaimAdmissionTest.test_noncanonical_artifact_stream_is_rejected_after_outer_reseal`; `AgentEquipmentDeploymentContractTests.test_preparation_bundle_binds_all_exact_pre_authorization_streams`; `AgentEquipmentDeploymentContractTests.test_preparation_receipt_is_closed_commit_evidence_not_apply_authority`; `AgentEquipmentDeploymentContractTests.test_apply_authorization_is_closed_over_the_complete_binding_tuple`; `AgentEquipmentDeploymentContractTests.test_apply_authorization_validates_the_complete_tuple_and_time_window`; `AgentEquipmentAcceptanceTest.test_compare_before_mutate_preserves_concurrent_changes_on_every_surface`; `AgentEquipmentAcceptanceTest.test_fresh_action_rejects_target_valued_concurrent_change`; `AgentEquipmentAcceptanceTest.test_auto_compensation_preserves_target_valued_external_change`; `AgentEquipmentAcceptanceTest.test_invocation_intent_write_failure_occurs_before_adapter_call`; `AgentEquipmentAcceptanceTest.test_plan_rejects_foreign_candidate_authority_before_checkpoint`; `AgentEquipmentAcceptanceTest.test_plan_rejects_ambiguous_capability_identity_before_checkpoint`; `AgentEquipmentAcceptanceTest.test_invalid_capability_binding_digests_fail_before_checkpoint`; `AgentEquipmentAcceptanceTest.test_checkpoint_replay_rejects_every_changed_binding`; `AgentEquipmentAcceptanceTest.test_checkpoint_replay_rejects_forged_embedded_step_identity`; `AgentEquipmentAcceptanceTest.test_checkpoint_preflight_rejects_malformed_durable_record`; `AgentEquipmentAcceptanceTest.test_checkpoint_preflight_rejects_duplicate_json_members`; `AgentEquipmentAcceptanceTest.test_checkpoint_binding_is_type_exact_for_json_values`; `AgentEquipmentAcceptanceTest.test_checkpoint_replay_binds_capability_set_and_route_evidence`; `AgentEquipmentAcceptanceTest.test_retry_preflights_every_checkpoint_binding_before_any_write`; `AgentEquipmentAcceptanceTest.test_completed_checkpoint_preserves_reverted_external_state`; `AgentEquipmentAcceptanceTest.test_completed_checkpoint_reverted_to_pre_state_blocks_all_compensation`; `AgentEquipmentAcceptanceTest.test_compensated_checkpoint_cannot_replay_forward_plan`. The production preparation and apply-preclaim seams prove exact byte, schema, digest, manifest, and cross-record binding before #116's final live comparison and nonce-claim composition. The model proves later checkpoint equality and replay rejection, not a production durable checkpoint implementation. |
| Partial `CHK-11` | `AgentEquipmentDeploymentContractTests.test_release_receipt_binds_launcher_authority_and_one_cas_archive`; terminal, partial-store, coordinated-reseal, and malformed-byte vectors in `AgentEquipmentDeploymentContractTests`. These pure vectors do not prove the external launcher's durable create-only archive implementation or release replay. |
| `CHK-04` through `CHK-06` state-machine examples, all automated mutating operations | `AgentEquipmentAcceptanceTest.test_forward_recovery_and_reverse_compensation_cover_every_mutating_operation` repeats post-mutation observation, completion-persistence recovery, and reverse compensation for every modeled mutating operation, including native-update suppression. It is not the complete `CHK-02` through `CHK-09` production matrix. |
...
Production command parsing and legacy-alias rejection are implemented. Live
discovery and source-resolution registries, adapter sandbox enforcement,
human-facing report rendering, and the complete real-adapter operation matrix
remain production-candidate requirements. The older fake acceptance model's
internal import/adoption names are not evidence for the current public command
contract. The fake migration matrix does not authorize or perform runtime
migration.

The fixture checkpoint store proves state-machine transitions, binding checks,
and fresh read-only status observation before retry. It does not model distinct
temporary-write, file-`fsync`,
