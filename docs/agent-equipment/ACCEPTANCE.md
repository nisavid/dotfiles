...
| `CMD-05` | `apply` rejects a missing, stale, replayed, expired, or incomplete authorization or plan before its first checkpoint, then reconciles every accepted catalog entry in deterministic order when both exact records are valid. | Partial: production authorization/schema/time-window rejection and durable ledger/CAS evidence plus model-only ordered plan trace; production executor/checkpoint evidence remains required |
...
| `ADP-01` | `AdapterContractSchemaTests.test_schema_is_valid_draft_2020_12`; `AdapterContractSchemaTests.test_valid_records_satisfy_the_closed_contract`; `AdapterContractSchemaTests.test_invalid_records_fail_closed`; `AdapterContractSchemaTests.test_individual_record_does_not_grant_mutation_authority`; `ImmutableContentSchemaTests.test_authoritative_and_installed_schemas_are_valid_and_digest_pinned` |
...
| Partial `CMD-05` | `ApplyAuthorizationTest.test_valid_authorization_is_claimed_once_after_complete_validation`; `ApplyAuthorizationTest.test_gate_prebinds_one_authoritative_domain_and_cas_target`; `ApplyAuthorizationTest.test_file_ledger_durably_claims_once_and_preserves_replay_evidence`; `ApplyAuthorizationTest.test_concurrent_claims_have_exactly_one_durable_winner`; `PlanActionSetAdmissionTest.test_complete_projection_is_admitted_as_one_immutable_typed_artifact`; `PlanActionSetAdmissionTest.test_missing_or_extra_actions_cannot_change_complete_membership`; `PlanActionSetAdmissionTest.test_trusted_set_digest_is_independent_of_the_supplied_projection`; `PlanActionSetAdmissionTest.test_each_exact_surface_identity_rule_admits_only_its_derived_set`; `PlanActionSetAdmissionTest.test_direct_constructor_cannot_bypass_complete_trust_membership`; `PlanActionSetAdmissionTest.test_legacy_projector_fails_closed_until_locator_authority_exists`; the apply-authorization deployment-contract tests; the invalid-final-action and fresh-home acceptance-model fixtures. Production proves closed authorization admission, authoritative execution-domain ownership, durable nonce CAS, and strict admission of a complete supplied plan-action-set against independent plan and set-digest trust for supported physical target kinds. Legacy-projector targets remain unavailable until their exact locator and route-slot authority are durably specified. Production projection of the artifact, executable-action authority, action checkpoints, and ordered reconciliation remain unproved. |
...
| Partial `CHK-01`; `CHK-02`, `CHK-03` | `ApplyAuthorizationTest.test_valid_authorization_is_claimed_once_after_complete_validation`; `ApplyAuthorizationTest.test_gate_prebinds_one_authoritative_domain_and_cas_target`; `ApplyAuthorizationTest.test_file_ledger_keeps_one_cas_target_after_path_replacement`; `ApplyAuthorizationTest.test_concurrent_claims_have_exactly_one_durable_winner`; `PlanActionSetAdmissionTest.test_complete_projection_is_admitted_as_one_immutable_typed_artifact`; `PlanActionSetAdmissionTest.test_coordinated_reseal_cannot_substitute_validated_action_authority`; `PlanActionSetAdmissionTest.test_physical_targets_require_the_canonical_identity_order`; `PlanActionSetAdmissionTest.test_direct_mcp_selection_contract_matrix_is_complete`; `PlanActionSetAdmissionTest.test_legacy_projector_fails_closed_until_locator_authority_exists`; the apply-authorization deployment-contract tests; `AgentEquipmentDeploymentContractTests.test_runtime_gate_and_external_authority_precede_every_mutation`; the invalid-plan and prepared-write acceptance-model fixtures. Production closes the authoritative execution-domain, durable authorization-ledger/CAS, and strict supplied plan-action-set admission portions of `CHK-01` for supported target kinds. Legacy-projector locator and route-slot authority, resolver-to-action-set projection, executable-action authority, zero-checkpoint integration rejection, durable action-checkpoint writes, retry classification, and adapter-call evidence remain required. |
...
| Partial `CHK-11` | `AgentEquipmentDeploymentContractTests.test_release_archive_manifest_and_receipt_bind_exact_bytes_and_execution`; `AgentEquipmentDeploymentContractTests.test_release_receipt_binds_launcher_authority_and_one_cas_archive`; terminal, partial-store, coordinated-reseal, and malformed-byte vectors in `AgentEquipmentDeploymentContractTests`. These pure vectors do not prove the external launcher's durable create-only archive implementation. |
