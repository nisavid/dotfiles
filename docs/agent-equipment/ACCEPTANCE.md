...
| `CAT-07` | Require every active or retiring route to have exactly one route control owner, exact provider-and-distribution-bound provenance owner, distribution-bound provider and restore evidence, restore class, activation group, native-update state, and disposition for every required operation. | Field-deletion, wrong-binding, destructive-target, and conflicting-field mutation fixtures |
...
| `CMD-02` | `unmanaged` reads runtime state and the authored catalog without changing either, then emits canonical secret-free observation records only for equipment positively observed on the machine and absent from the catalog. It excludes cataloged operator-owned routes, and every runtime observation remains factual rather than a proposal. | Observation-record golden, operator-owned exclusion, recursive secret-canary scan, and runtime/catalog digest equality |
| `CMD-03` | `add` requires no prior `unmanaged` invocation. In the same invocation it freshly observes targeted unmanaged equipment, revalidates the state and every binding, and emits one atomic proposed catalog addition without changing runtime state. Any changed state or binding fails with no partial proposal. | Fresh-observation call trace, binding-change injection matrix, atomic catalog diff, and runtime digest equality |
| `CMD-04` | `update` follows the configured source tracking policy, expands source-wide selection, advances immutable targets or reviewed rolling baselines, and emits one atomic proposed catalog-and-resolved-lock update without installing or otherwise changing runtime state. | Source-policy fixtures, catalog-and-lock diff, and runtime digest equality |
...
| Partial `CMD-01`; `CMD-02` and `CMD-03` unproved | `StatusCliTests.test_exact_status_ignores_xdg_and_resolves_reviewed_home_inputs_read_only`; `ReadOnlyCollectorTests.test_collector_calls_only_capabilities_and_observe`; and `ReadOnlyCollectorTests.test_collector_returns_no_partial_inventory_after_an_exception` prove the current read-only status and collection seams. Production fixtures must still prove rejection of every legacy command alias, the unmanaged definition and operator-owned exclusion, canonical secret-free records, and `add` fresh observation, revalidation, atomicity, and no-prior-invocation contract. |
