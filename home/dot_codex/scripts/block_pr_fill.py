...
try:
    ...
    for index in range(2, len(sys.argv), 8):
        (
            source_path,
            cache_path,
            optimization,
            ...
            cache_device,
            cache_inode,
        ) = sys.argv[index : index + 8]
        with open(source_path, "rb") as source_file:
            source_status = os.fstat(source_file.fileno())
            if (
                source_status.st_dev != int(source_device)
                or source_status.st_ino != int(source_inode)
            ):
        ...
        with open(cache_path, "rb") as cache_file:
            ...
            if (
                status.st_dev != int(cache_device)
                or status.st_ino != int(cache_inode)
            ):
...
TRUSTED_REVIEW_STATE_LEDGER_TARGETS = {
    ("nisavid/agents", "28"),
    ("nisavid/agents", "29"),
}
...
def _review_state_invocation_is_invalid(segment: list[str]) -> bool | None:
    ...
    write_ledger_is_scoped = "--write-ledger" not in flags or (
        repository,
        pr_number,
    ) in TRUSTED_REVIEW_STATE_LEDGER_TARGETS
    return not (
        values.keys() == {"--repo", "--pr"}
        and flags
        in (
        ...
        and int(pr_number) > 0
        and write_ledger_is_scoped
    )


def _pinned_file_identity(path: Path, expected_digest: str) -> tuple[int, int] | None:
    try:
        resolved = path.resolve(strict=True)
        if path.is_symlink():
            return None
        with resolved.open("rb") as handle:
            ...
            if (
                ...
                or status.st_size > MAX_TRUSTED_BYTECODE_BYTES
            ):
                return None
        ...
        if (
            ...
            or path_status.st_dev != status.st_dev
            or path_status.st_ino != status.st_ino
        ):
            return None
    except OSError:
        return None
...
def _review_state_helper_is_trusted() -> bool:
    return _pinned_file_identity(REVIEW_STATE, REVIEW_STATE_SHA256) is not None


def _trusted_help_only_invocation(  # noqa: PLR0911, PLR0912
    segment: list[str],
) -> bool:
    if (
        len(segment) != 3
    ...
    try:
        ...
        unexpected = entries - set(TRUSTED_HELP_ONLY_SCRIPTS) - {cache_directory}
        for candidate in unexpected:
            status = candidate.lstat()
            if (
                candidate.name not in IGNORABLE_TRUSTED_DIRECTORY_ENTRIES
        ...
        for candidate, candidate_digest in TRUSTED_HELP_ONLY_SCRIPTS.items():
            identity = _pinned_file_identity(candidate, candidate_digest)
            if identity is None:
                return False
            source_identities[candidate] = identity
...
def _trusted_bytecode_cache(  # noqa: PLR0911, PLR0912
    directory: Path,
    ...
    try:
        for candidate in directory.iterdir():
            ...
            if source is None:
                return False
            source_identity = source_identities.get(source)
...
def _git_blob_digest(path: Path, status: os.stat_result) -> str | None:
    # Git's object format requires SHA-1; this is not a security digest.
    digest = hashlib.sha1(usedforsecurity=False)  # noqa: S324
    digest.update(f"blob {status.st_size}\0".encode("ascii"))
    try:
...
def _working_tree_matches_git_tree(  # noqa: PLR0911, PLR0912
    root: Path, tree_path: Path
) -> bool:
    try:
    ...
    expected: dict[Path, tuple[bytes, str, int]] = {}
    ...
    try:
        for directory, directories, filenames in os.walk(
            ...
            for name in filenames:
                ...
                expected_mode, expected_digest, expected_size = entry
                if (
                    mode != expected_mode
                    or status.st_size != expected_size
                    or _git_blob_digest(candidate, status) != expected_digest
                ):
                    return False
                actual.add(relative)
...
def _command_assessment(
    ...
    for segment_number, segment in enumerate(_segments(command_without_data)):
        ...
        if executable_name in {
            ...
            and segment[executable_index] == "."
        ):
            shell_lookup_mutated = True
        ...
        if dynamic_route is not None:
            return True, dynamic_route
        owned_lookup_is_trusted = allow_owned_helpers and not shell_lookup_mutated
        if (
            owned_lookup_is_trusted
            and depth == 0
            and direct_python
            and not bindings
            and _trusted_help_only_invocation(segment)
        ):
            # This guard proves static publication routes, not host integrity.
            # Ambient PATH and interpreter startup files share the operator's
            # trust boundary with the hook itself; inline overrides are still
            # parsed and rejected before this exact helper exemption.
            continue
        ...
        if (
            ...
            # The literal launcher is part of the operator-controlled host
            # boundary. Repository identity, the pinned tree, and every live
            # file in that tree are independently proven above.
            continue
