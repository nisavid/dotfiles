...
def _javascript_known_source_expression_symbols(
    ...
    *,
    ...
    if member is not None:
        ...
        source_symbol = member.group("root")
    else:
        getter = re.fullmatch(
        ...
        source_symbol = getter.group("root") or getter.group("function")
        source_is_safe = True
    if (
...
def _lex_javascript(
    ...
    def span_sort_key(span: tuple[int, int, str]) -> tuple[int, int]:
        return span[0], span[1]

    return (
        tuple(tokens),
        tuple(sorted(opaque_spans, key=span_sort_key)),
...
def _javascript_lexical_index(
    ...
    *,
    ...
    for opening, token in enumerate(tokens):
        ...
        previous_token = tokens[opening - 1] if opening else None
        if previous_token is None or previous_token.text in controls:
            continue
        after = closing + 1
        ...
        if (
            body is not None
            and previous_token.kind == "identifier"
            and previous_token.text != "function"
        ):
            parameter_lists.add((opening, closing))
            if not previous_is_function_name:
                ignored_symbol_positions.add(previous_token.start)
...
def _static_javascript_string_expression(value: str) -> str | None:
    ...
    while index < len(candidate):
        if candidate[index] in {"'", '"', "`"}:
            ...
            literal = parsed[0]
            if candidate[index] == "`":
                template_literal = _static_javascript_template_key(literal)
...
def _javascript_mappings_contain_literal_credential(
    ...
    *,
    ...
    if _javascript_direct_assignment_contains_credential_worthy_literal(
        ...
        return True
    for direct_key, match_start, match_end in _javascript_direct_members(
        value,
        ...
        if _credential_field_identity(
            ...
            candidates.append((direct_key, match_end))
    for computed_key, start, colon in _javascript_computed_members(
        value,
        lexical_index,
    ):
        if not _javascript_member_is_nonvalue(lexical_index, colon):
            candidates.append((computed_key, start))
    candidates.extend(lexical_index.typed_assignment_members)
    budget = len(value) * 4 + 4096
    allowed_ranges_by_name: dict[str, list[tuple[int, int]]] = {}
    for candidate_key, start in candidates:
        expression, consumed = _javascript_bounded_mapping_expression(
            value,
            start,
            lexical_index,
        ...
        if candidate_key is None:
            if _javascript_unresolved_computed_member_requires_review(expression):
                return True
            continue
        symbols = _javascript_mapping_source_symbols(candidate_key, expression)
        if symbols is None:
            return True
        expression_start = _javascript_trivia_end(value, start)
...
def _decode_javascript_unicode_escape(match: re.Match[str]) -> str:
    ...
    *,
    ...
    if templated:
        ...
        if projection is None:
            return True
        value = projection
        if syntax is not None and syntax != "template":
            syntax = syntax.removesuffix("-template")

    candidate = _JSON_UNICODE_ESCAPE.sub(_decode_json_unicode_escape, value)
    candidate = _JS_BRACED_UNICODE_ESCAPE.sub(
        _decode_javascript_unicode_escape,
        candidate,
    )
    if (
