...
Before any live probe, require the task to authorize the route's account, data boundary, workspace, tools, the probe itself, and any external actions. If that authority is absent, classify the route as unavailable without probing it.

A route is genuinely runnable only when that authority is established, its current selector exposes the exact Daybreak model, the bound account is authenticated, usage capacity remains, and one harmless probe containing no task data succeeds. Picker visibility, model selectability, authentication, capacity, session start, and successful execution are separate facts. Resolve the exact exposed model; never invent a slug or reuse a stale one.

Probe each exact invocation surface, account home when applicable, identity, model, and capability-state tuple at most once per task. A failure applies only to that tuple. A concrete capacity, entitlement, authentication, model, selector, authority, data-boundary, workspace, or tool-scope change creates a new tuple eligible for one probe. Retain redacted evidence without credentials, account IDs, account-home names, or task data.
