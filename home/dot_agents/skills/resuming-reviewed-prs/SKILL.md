...
Use this when picking up a reviewed PR and moving it toward merge without short-circuiting reviewer trust. Refresh the branch, evaluate every comment against the current diff, fix and tighten what belongs in scope, then stop with a packet Ivan can review before any replies or thread actions are taken.

Core principle: make the PR easier to approve without hiding new behavior or acting on GitHub before the handling plan is visible.
...
- This skill's pause-packet rule is stricter than thread-resolution permissions in `pr-review-orchestration`; pause first unless Ivan explicitly grants actuation for this run.
- Use `shepherd-pr` instead only when Ivan asks for autonomous PR maintenance rather than a pause packet.
...
Assume draft-only GitHub authority by default. Local edits, commits, ordinary pushes, force-pushes after rebase, posting replies, resolving threads, marking ready, and merging each need support from the current instruction, repo policy, or an explicit Ivan decision. "Rebase it" authorizes the local rebase; it does not by itself authorize force-pushing the remote branch.
...
11. Prepare the pause packet and stop. Do not post replies, resolve threads,
    mark ready, or merge before Ivan reviews it.
...
- Draft replies as Ivan in first person, but do not post them until approved.
- Do not resolve reviewer-owned threads unless ownership and permission are clear for this PR.
...
| Posting because the draft looks ready | Stop with the pause packet and wait for Ivan. |
