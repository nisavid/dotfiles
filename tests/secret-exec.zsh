...
[[ $1 == lookup && $# == 7 ]] || exit 64
[[ $2 == application && $3 == secret-exec ]] || exit 65
[[ $4 == profile && $5 == member-local ]] || exit 66
[[ $6 == name && $7 == SITE_PASSWORD ]] || exit 67
...
output=$(zsh "$launcher" member-local -- check-secret-service)
[[ $output == secret-service-ok ]] || fail 'Secret Service profile must reach the target'
[[ $(<"$FAKE_SECRET_TOOL_LOG") == \
  'lookup application secret-exec profile member-local name SITE_PASSWORD' ]] || \
