...
required=(
  ...
  'and only when the credit is load-bearing for the point rather than freestanding praise'
  'A detail earns its place only if it changes what the reader does next.'
  'if two pieces share an opening, a closing move, or roughly the same length, rewrite one from its own content'
