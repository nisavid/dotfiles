...
if [[ "$(uname -s)" != Darwin ]]; then
  print -r -- "hindsight harness reconcile wrapper: SKIP (Darwin-only binding)"
  exit 0
fi
...
managed_python_relative="$(fixture_value managedPython)"
install_root_relative="$(fixture_value installRoot)"
state_root_relative="$(fixture_value stateRoot)"
harness_reconciliation_relative="$(fixture_value harnessReconciliationPath)"
managed_python="$(command -v python3)"
