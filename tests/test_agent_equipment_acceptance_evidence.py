...
if __package__:
    from tests.agent_equipment_acceptance_evidence_fixtures import (
        CANDIDATE_IDENTITY,
        DIGESTS,
        EXECUTION_BINDING,
        evidence_for,
        ...
        valid_attestation_manifest,
        valid_evidence_bundle,
        valid_expected_case_manifest,
    )
else:
    from agent_equipment_acceptance_evidence_fixtures import (
        CANDIDATE_IDENTITY,
        DIGESTS,
        EXECUTION_BINDING,
        evidence_for,
...
class AcceptanceEvidenceContractTests(unittest.TestCase):
    ...
    def test_attestation_must_follow_results_and_bind_the_live_signer(self) -> None:
        ...
        for label, (timestamp, expected_codes) in timestamp_variants.items():
            with self.subTest(timestamp=label):
                ...
                self.assertEqual(
                    diagnostic_codes(bundle, manifest, variant),
                    expected_codes,
                )
    ...
    def test_public_validator_accepts_the_complete_closed_evidence_projection(
        ...
        self.assertEqual(len(requirement_ids()), 75)
        self.assertIn("CHK-11", requirement_ids())
        self.assertEqual(len(expected_cases(manifest)), 93)
        self.assertEqual(
            [
                case["evidence_kind"]
                for case in manifest["static_cases"]
                if case["requirement_id"] == "CHK-11"
            ],
            ["checkpoint_trace"],
        )
